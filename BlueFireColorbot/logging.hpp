...
namespace logging {
	...
	enum class log_level : uint8_t { TRACE = 0, DEBUG = 1, INFO = 2, WARN = 3, CRITICAL = 4 };
	...
	{
	  {log_level::CRITICAL, " [ERROR] "}, {log_level::WARN, " [WARN] "}, {log_level::INFO, " [INFO] "},
	  {log_level::DEBUG, " [DEBUG] "}, {log_level::TRACE, " [TRACE] "}
	};
	const std::unordered_map<log_level, std::string, enum_hasher> colored
	{
	  {log_level::CRITICAL, " \x1b[31;1m[ERROR]\x1b[0m "}, {log_level::WARN, " \x1b[33;1m[WARN]\x1b[0m "},
	...
	inline void CRITICAL(const std::string& message) {
		get_logger().log(message, log_level::CRITICAL);
	}
}
